...
#        (Open Source Raumlufttechnik-Simulation mit diskretem PI-Regler
#
#  Dieses Programm simuliert das thermodynamische Verhalten eines
#  vereinfachten Raummodells mit vollständiger diskreter PI-Reglung der
...
from pi_regler_diskret import DiskreterPIRegler           # Regler mit Übertragungsfunktion
...
n_BFT = config["befeuchter"]["n_BFT"]                    # Befeuchterkonstante: kg Wasser pro kg Luft
...
arbeitspunkt_X = X_SOL_ZUL                                # Arbeitspunkt für die Feuchteregelung
regler_X_ZUL = DiskreterPIRegler(                         # Diskreter PI-Regler für Zuluftfeuchte
    config["regler"]["X_ZUL"]["kp"],
    config["regler"]["X_ZUL"]["ti"],
    config["regler"]["X_ZUL"]["dt"]
)

regler_HUM = PIRegler(                                    # Kontinuierlicher PI-Regler für Befeuchter (m_HUM)
    config["regler"]["BFT"]["kp"],
    config["regler"]["BFT"]["ki"],
    dt
)

regler_TEP = DiskreterPIRegler(                           # Diskreter PI-Regler für Heiz-/Kühlmassenstrom (m_TEP)
    config["regler"]["TEP"]["kp"],
    config["regler"]["TEP"]["ti"],
    config["regler"]["TEP"]["dt"]
)

arbeitspunkt_T = T_SOL_ZUL                                # Arbeitspunkt für Temperaturregelung (Zuluft)
regler_T_ZUL = DiskreterPIRegler(                         # Diskreter PI-Regler für Zulufttemperatur
...
vis = Visualisierung()                                    # Erstellt Visualisierungsobjekt zur Datenspeicherung und Darstellung
...
# =============================
    if i == config["simulation"]["stoerung_intervall"]:  # Wenn der Störintervall erreicht ist, dann wird eine neue Störung eingeleitet
...
#     Ventilatorsteuerung
# =============================
    T_SOL_ZUL = regler_T_ZUL.update(T_SOL_ZUL, T_SOL_R, T_R)                      # Berechne Soll-Zulufttemperatur mit Regler mit Übertragungsfunktion
    X_SOL_ZUL = regler_X_ZUL.update(X_SOL_ZUL, X_SOL_R, X_R)                      # Berechne Soll-Zuluftfeuchte mit Regler mit Übertragungsfunktion
...
# =============================
    if dT_RA_SOL > config["schwellenwerte"]["dT_RA_SOL"]:                         # Nur regeln, wenn Temperaturabweichung im Raum groß genug
        ...
        m_TEP_roh = regler_TEP.update(m_TEP_roh, T_SOL_ZUL, T_ZUL)                        # Diskreter PI-Regler berechnet Rohwert für Heiz-/Kühlleistung
...
# =============================
    ...
    if t % 10 == 0:                                                                   # Ausgabe in jeder 10. Iteration
...
# Visualisierung
    vis.add_data(                                                                     # Übergibt aktuelle Werte an das Visualisierungsobjekt
        t, T_SOL_R, T_R, T_ZUL, T_SOL_ZUL, T_WRG, m_ERH, m_KUL,
        m_LUF, X_R, X_SOL_R, X_SOL_ZUL, X_ZUL, m_BFT, m_ENF, wrg_on
    )
